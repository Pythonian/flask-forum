...
class Thread(db.Model):
    ...
    def last_post_date(self):
        """
        Return the date of last reply if there is a reply
        or return the date the thread was created if no reply.
        """
...
def thread(thread_id):
    ...
    if form.validate_on_submit():
        reply = Reply(user_id=current_user.id,
                      message=form.message.data, date_created=datetime.now())
        # apply the reply to the list of replies
    ...
    return render_template('thread.html', thread=thread, form=form,
                           replies=replies, current_user=current_user)
